...
{
  char *params;
  params = malloc(STACK_SIZE); /* free in merge_unpack */
  if (params == NULL) exit(-1);	/* not the best feedback */
  sprintf(params, "%d %d", start, end);
  return (void *)params;
}
...
void merge(int start, int end)
{
  ...
  if (end - start + 1 < 2)
    ...
    MyThreadCreate(merge_unpack, merge_pack(mid + 1, end));   // sort 2nd half
    MyThreadJoinAll();
    k = start;
    j = mid + 1;
    int pos = 0;
    int* tmp = calloc(end-start+1, sizeof(int));
    while (k <= mid && j <= end) {
      if (List[k]<List[j]) {
        tmp[pos++] = List[k++];
      } else {
...
void initializeList(int n, int* List) {
  int i;
  for (i = 0; i < n; i++) {
    List[n-i-1] = i;
  }
